...
#define LINELENGTH 1024
	...
	/* keywords */
enum l_keyword {k_birth, k_death, k_marriage, k_divorce,
		k_cohab_prob, k_birthtarget, k_transittarget, k_take_census,
		k_single, k_married, k_divorced, k_widowed, k_cohabiting,
		k_input_file, k_output_file, k_execute, k_include, k_done,
		k_female, k_male, k_run, k_end, k_segments, k_sex_ratio,
		k_proportion_male,
		k_float, k_integer, k_double, k_string, k_read_xtra,
		k_size_of_xtra,
		k_lc_init, k_lc_ax, k_lc_bx, k_lc_k_val, k_lc_k_mean, 
		k_lc_k_std_dev,
		k_lc_k_start_list, k_lc_epsilon, k_fix_epsilon, 
		k_random_epsilon, k_random_father, k_random_father_min_age,
		k_alpha, k_beta, k_bint, k_duration_specific, k_factor,
		k_hhmigration, k_hetfert, k_endogamy, k_duration,
		k_transit, k_marriage_peak_age, k_marriage_slope_ratio,
		k_marriage_agedif_max, k_marriage_agedif_min,
		k_agedif_marriage_mean,k_agedif_marriage_sd,
		k_marriage_eval,k_marriage_queues,
		k_firstyear, k_hettrans, k_write_output,k_child_inherits_group,
		k_parameter0,k_parameter1,k_parameter2,
		k_parameter3,k_parameter4,k_parameter5,
		k_parameter6,k_parameter7,k_parameter8,
		k_parameter9,k_parameter10,
		k_unknown };
...
char *index_to_sex[] = {"male", "female",};
char *index_to_event[] =
    {"birth", "marriage", "divorce", "death", "transit1", "transit2",
      "transit3","transit4","transit5","transit6","transit7",
     "transit8","transit9","transit10","transit11","transit12",
     "transit13","transit14","transit15","transit16", "transit17",
     "transit18", "transit19", "transit20", "transit21", "transit22",
     "transit23", "transit24", "transit25", "transit26", "transit27",
     "transit28", "transit29", "transit30", "transit31", "transit32",
     "transit33", "transit34", "transit35", "transit36", "transit37",
     "transit38", "transit39", "transit40", "transit41", "transit42",
     "transit43", "transit44", "transit45", "transit46", "transit47",
     "transit48", "transit49", "transit50", "transit51", "transit52",
     "transit53", "transit54", "transit55", "transit56", "transit57",
     "transit58", "transit59", "transit60",
     "null",};
char *index_to_mstatus[] = 
    {"null", "single", "divorced", "widowed", "married", "cohabiting",};
char command[80];
	...
	char *file;
{
  ...
  if (current_fstatus == OPEN) {
    ...
    if ((fp = fopen(file, "r")) == 0) {
      ...
      logmsg("Can't open ratefile: %s",file,1);
		...
		*/
		{
			...
			if (nwords == 3) {
				...
				l_error(cx, "Not a valid rate line");
				return -1;
			    } else if ((years + (int) months/12 > MAXUYEARS)
					|| (months < 0)) {
				l_error(cx, "Not a valid rate line");
				return -1;
			    }
	...
	*/
	...
	case  k_marriage_queues :
	  ...
	  if(marriage_queues != 1 &&  marriage_queues != 2){
	    l_error(cx,"marriage_queues out of range\n");
	    exit(-1);
	  }
	...
	case  k_marriage_eval :
	  if(strcmp(words[1], "preference")==0){
	    ...
	    l_error(cx,"ERROR:marriage_eval parameter makes no sense\n");
	    exit(-1);
	  }
	...
	case  k_agedif_marriage_mean:
	  if(nwords != 3){
	    l_error(cx,"agedif_marriage_mean : malformed wrong number of parameters\n");
	    exit(-1);
	  }
	  group=read_integer(cx,words[1]);
	  ...
	  if(agedif_marriage_mean[group] >  MAXUYEARS ||
	    ...
	    l_error(cx, "agedif_marriage_mean gr out of bounds\n");
	    exit(-1);
	  }
	  return 1;
	    break;
	  ...
	  if(nwords != 3){
	    l_error(cx,"agedif_marriage_sd : malformed wrong number of parameters\n");
	    exit(-1);
	  }
	    ...
	    if(agedif_marriage_sd[group] < 0){
	      ...
	      l_error(cx, "agedif_marriage_sd less than 0");
	      exit(-1);
	    }
	    return 1;
	    break;
	  ...
	  case  k_parameter2:
	    ...
	    case k_alpha:
	      ...
	      if(alpha > 1 || alpha < 0){
		l_error(cx,"ERROR:alpha out of bounds 0<=alpha<=1\n");
		exit(-1);
	      }
	      logmsg("%s\n",logstring,0);
	      return 1;
	      break;
	    case k_beta:
	      sscanf(words[1], "%lf", &beta);
	      sprintf(logstring,"beta %lf\n", beta);
	      logmsg("%s\n",logstring,0);
	      return 1;
	      break;
	    case k_bint:
		...
		numgroups=(child_inherits_group > numgroups)?
...
\n        from_same_sex_parent\n           from_opposite_sex_parent or group\n");
	    ...
	    case k_transit:
	      ...
	      current_block = NEW(struct age_block);
	      current_block->previous = NULL;
	      rate_set[group][event][sex][mstatus]
		= current_block;
	      recall = current_block;
	      /* destination of transit rate implies group existance*/
	      numgroups=(group > numgroups)?group:numgroups;
    ...
    register char *word;
{
    ...
    static struct tab tab[] = {
	    ...
	    "divorce", k_divorce,
	    "marriage", k_marriage,
	    "cohab_prob", k_cohab_prob,
	    "birthtarget", k_birthtarget,
	    "transittarget", k_transittarget,
	    "take_census", k_take_census,
	    "sex_ratio", k_sex_ratio,
	    "proportion_male", k_sex_ratio,
	    "input_file", k_input_file,
	    "output_file", k_output_file,
	    "execute", k_execute,
	    "include", k_include,
	    "end", k_end,
	    "single", k_single,
	    ...
	    "done", k_done,
	    "run", k_run,
	    "read_xtra", k_read_xtra,
	    "size_of_xtra", k_size_of_xtra,
	    "float", k_float,
	    "lc_init", k_lc_init,
	    "lc_ax", k_lc_ax,
	    "lc_bx", k_lc_bx,
	    "lc_k_val", k_lc_k_val,
	    "lc_k_mean", k_lc_k_mean,
	    "lc_k_std_dev", k_lc_k_std_dev,
	    "lc_k_start_list", k_lc_k_start_list,
	    "lc_epsilon", k_lc_epsilon,
	    "fix_epsilon", k_fix_epsilon,
	    "random_epsilon", k_random_epsilon,
	    "random_father", k_random_father,
	    "random_father_min_age", k_random_father_min_age,
	    "integer", k_integer,
	    "double", k_double,
	    "string", k_string,
	    "alpha", k_alpha,
	    "beta", k_beta,
	    "bint", k_bint,
	    "duration_specific", k_duration_specific,
	    "factor", k_factor,
	    "hhmigration", k_hhmigration,
	    "hetfert", k_hetfert,
	    "endogamy", k_endogamy,
	    "write_output", k_write_output,
	    "child_inherits_group", k_child_inherits_group,
	    "duration", k_duration,
	    "firstyear", k_firstyear,
	    "transit", k_transit,
	    "hettrans", k_hettrans,
	    "agedif_marriage_mean" ,k_agedif_marriage_mean,
	    ...
	    "marriage_agedif_max", k_marriage_agedif_max,
	    "marriage_agedif_min", k_marriage_agedif_min,
	    "parameter0", k_parameter0,
	    "parameter1", k_parameter1,
	    "parameter2", k_parameter2,
	    "parameter3", k_parameter3,
	    "parameter4", k_parameter4,
	    "parameter5", k_parameter5,
	    "parameter6", k_parameter6,
	    "parameter7", k_parameter7,
	    "parameter8", k_parameter8,
	    "parameter9", k_parameter9,
	    "parameter10", k_parameter10,
	    0, k_unknown,
    };
...
l_create_rateset(cx, name)
    register struct l_context *cx;
    char *name;
{
    cx->file = name;
...
*/
	...
	for (g = 1; g <= numgroups; g++) {
		...
		lc = lc_rate_set[g][s];
		prod1 = lc->ck[AX] * lc->ck[BX] * lc->ck[K_VAL] * lc->ck[K_MEAN]
		    * lc->ck[K_STD_DEV];
		...
		if (prod1 + prod2 < 1) {
			...
			*/
			lc->mean = (read_kt_vals[g][s][last_kt_index[g][s]]
			    - read_kt_vals[g][s][0]) / last_kt_index[g][s];
			printf("computed mean %lf\n", lc->mean);
			lc->std_dev = 0;
			for (current_kt_index[g][s] = 0;
				current_kt_index[g][s]
				    < MIN(MAXUYEARS, last_kt_index[g][s]);
				current_kt_index[g][s]++) {
...
warn_about_rates(zero_block,zero_fert_block)
     struct age_block *zero_block, *zero_fert_block;
{
...
**/
  ...
  for(g=1;g<=numgroups;g++){
	...
	if(p==0 & birth_rate_set[g][m][1] != NULL){break;}
