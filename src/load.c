 ...
 * Process a line
 */
int l_process_line(line, cx, fp) char *line;
struct l_context *cx;
FILE *fp;
{
...
/*
	   ...
	   */
l_scan_line(line, words) char *line;
char **words;
{
...
enum l_keyword
	l_lookup_keyword(word) register char *word;
{
	/**************************************************************
...
l_create_rateset(cx, name) register struct l_context *cx;
char *name;
{
	cx->file = name;
...
add_rate_table(years, months, prob) int years, months;
double prob;
{
...
add_rate_block(years, months, prob) int years, months;
double prob;
{
...
add_lc_rate_block(years, months, prob) int years, months;
double prob;
{
	if (read_ax_or_bx == BX)
	{
...
int
	read_integer(cx, w) struct l_context *cx;
char *w;
{
	/*
...
int
	keyword_to_integer(cx, k) struct l_context *cx;
enum l_keyword k;
{
	switch (k)
	{
...
warn_about_rates(zero_block, zero_fert_block) struct age_block *zero_block, *zero_fert_block;
{
...
print_prob_table(t) struct age_table *t;
{
...
error(fmt, a, b, c, d, e, f) char *fmt;
char *a;
int b;
char *c;
{
