...
char * a_tally_to_string[] = { "SELF", "SPOU", "BKID", "SKID", "BSONBKID",
    "BDAUBKID", "BSONSKID", "BDAUSKID", "SSONBKID", "SDAUBKID",};

char * c_tally_to_string[] = { "SELF", "SPOU", "BDAD", "BMOM", "SDAD",
    "SMOM", "BPARBKID", "BDADBKID", "BMOMBKID", "BDADBDAD", "BDADBMOM",
    "BMOMBDAD", "BMOMBMOM", "BDADSDAD", "BDADSMOM", "BMOMSDAD",
    "BMOMSMOM", "SDADBDAD", "SDADBMOM", "SMOMBDAD", "SMOMBMOM",
    "BDADBDADBDAD", "BDADBDADBMOM", "BDADBMOMBDAD", "BDADBMOMBMOM",
    "BMOMBDADBDAD", "BMOMBDADBMOM", "BMOMBMOMBDAD", "BMOMBMOMBMOM",
    "BDADBPARBKID", "BDADBDADBKID", "BDADBMOMBKID", "BMOMBPARBKID",
    "BMOMBDADBKID", "BMOMBMOMBKID", "BDADBDADBPARBKID", "BDADBMOMBPARBKID",
    "BMOMBDADBPARBKID", "BMOMBMOMBPARBKID", "NUMCAT",};
...
char *argv[];
{
    ...
    */
    ...
    if (argc < 3 ) {
      sprintf(useage,"useage: %s ratefile random_number", argv[0]);
      perror(useage);
      exit(1);
    }
    ...
    *****************************/
    ...
    if(load(rate_file_name) < 0) exit(-1);
	...
	agedif_marriage_mean[g]=  ( agedif_marriage_mean[g]== -99) ?
	  agedif_marriage_mean[g-1]: agedif_marriage_mean[g];
	agedif_marriage_sd[g]=  ( agedif_marriage_sd[g]== -99) ?
	  agedif_marriage_sd[g-1]: agedif_marriage_sd[g];
    ...
    */
    ...
    while (current_segment <= num_segments) {
      ...
      if (write_output) prepare_output_files(current_segment);
...
initialize_marriage_targets()
{
  ...
  for(g=0;g<MAXGROUPS;g++){
    for(i=0;i<(2*MAXUYEARS);i++){
      marriage_agetarget[g][i]= (marriage_agetarget[g][i]>0)? 
	marriage_agetarget[g][i]/norm_tots[g]:
     ...
     FILE *fd;
{
	...
	return;
      ...
      for (p = e->first, i = 1; i < e->num; p=NEXT_ELEMENT(p), i++) {

	fprintf(fd,"%d : %d\t", p->person_id, p->group);
    ...
    int q_type;
{
    ...
    */
	...
	printf("in queue_delete, before deletion: inspecting entry\n");
...
**/
modify_rates(event, first_block, p) 
    int event;
...
int
datev(first_block, age, time_shift)
    struct age_block * first_block;
    int age, time_shift;
{
    ...
    struct person *p;
{
    ...
    /*
    printf("entering date_and_event\n");
  ...
  if ((p->NEXT_ON_MQUEUE == NULL || marriage_queues == 1) &&
    ...
    */
	...
	if ((first_block =
	  ...
	  if(
	     (duration_specific[p->group][NONTRANSIT + i][p->sex][p->mstatus])
	     && (p->ltrans != NULL)) {
...
FILE *fd_pyr;
{
    ...
    */
    ...
    for (g = 0; g <= NUMBER_OF_GROUPS; g++) {
      ...
      */   
      /* fprintf(fd_pyr, "\f") */
	if (sum[MALE][g] + sum[FEMALE][g] == 0) continue;
	if (sum[MALE][g] == 0) continue;
	if (sum[FEMALE][g]==0) continue;
	...
	*/
	...
	/** only room for 40% in  any bar -- if the population (init for 
	  ...
	  for(j =1; j<=39;j++) strcat(leftline,"M");
	}
	...
	*/
	...
	if(num1+num2+num3 <39){
	  ...
	  /** only room for 40% in  any bar -- if the population (init for 
	      example) is all babies we need to avoid the fancy bars**/
	  for(j =1; j<=39;j++) strcat(rightline,"F");
	}
     ...
     FILE *fd;
{
  ...
  */
  ...
  if((alpha !=0) || (beta !=1)){
    ...
    fprintf(fd,"Fert Mult inheritance: alpha=%f beta=%f\n",alpha,beta);
  }else{
    fprintf(fd,"no\n");
  }
  ...
  for (g = 1; g <= numgroups; g++) {
    for (m = 0; m < NUMMARSTATUS; m++) {
      if(birth_rate_factors[g][m] != 1) count++;
    }
  }
  if(count){ fprintf(fd,"yes\n");} else {fprintf(fd,"no\n");};
