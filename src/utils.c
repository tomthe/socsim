...
int
get_expected_number_of_births(group)
int group;
{
...
int
get_expected_number_of_transits(from, dest)
int from, dest;
{
...
fill_in_rates(rate_table, rate_set)
double *rate_table;
...
double usedeathmult(p, crnt_block)
     struct person *p;
     struct age_block * crnt_block;
{
  /** 
...
census() {
    ...
    */
	...
	if (mother != NULL) {
		...
		if ((stepfather = mother->last_marriage->husband) != father) {
			...
			*/
		    ...
		    if ((stepfather->mother != NULL)
			&& (CHECK(stepfather->mother))) {
			/*
	...
	if (father != NULL) {
		...
		if ((stepmother->mother != NULL)
		    && (CHECK(stepmother->mother))) {
		    /*
...

{
  ...
  **/
  ...
  pmar = (p->sex == FEMALE? cmar->wifes_prior :
	  cmar->husbands_prior);
  ...
  while(pmar != NULL){
    ...
    if(mstat_found == 1){
		  ...
		  pmar = (p->sex == FEMALE? pmar->wifes_prior :
			  pmar->husbands_prior);
		}
  }/*while*/
  return(prev_mstat);
...
/******************************************************************/
int get_previous_mstatusOBSOLETE(p)
     struct person *p;
{
