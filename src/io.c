...
void logmsg(frmt,msg,where)
     char msg[1024], frmt[1024];
     int  where;
{
...
struct nlist *
install(uniontag, val, p, i1, i2, i3, i4, i5, i6)
    int uniontag, val;
    void *p;
{
    ...
    if ((np = lookup(uniontag, val)) == NULL) {    /* not found */
	np = NEW(struct nlist);
	hashval = hash(val);
...
/**  **/
void write_popfiles(intermediate)
  int intermediate;
{
